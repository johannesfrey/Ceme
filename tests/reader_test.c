...
int test_reader()
{
    ...
    check(obj->u.any.tag == T_NUMBER, "Not of type T_NUMBER");
    ...
    obj = read_object(input);
    check(obj->u.any.tag == T_STRING, "Not of type T_STRING");
    ...
    obj = read_object(input);
    check((obj->u.any.tag == T_TRUE && obj == true_object), "Not of type T_TRUE");

    check(input = write_to_testfile("#f"), "write_to_testfile failed");
    obj = read_object(input);
    check((obj->u.any.tag == T_FALSE && obj == false_object), "Not of type T_FALSE");

    check(input = write_to_testfile("nil"), "write_to_testfile failed");
    obj = read_object(input);
    check((obj->u.any.tag == T_NIL && obj == nil_object), "Not of type T_NIL");
